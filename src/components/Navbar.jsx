...
//             <li
//               key={index}
//               className="relative group cursor-pointer hover:text-pink-500 flex items-center gap-1"
//             >
              
//               <Link to={`/category/${item.slug}`}>{item.name}</Link>
//                 {item.subcategories && (
//                 <ChevronDown className="w-4 h-4 mt-0.5 group-hover:rotate-180 transition duration-200" />
//               )}

//               {/* Dropdown */}
//               {item.subcategories && (
//                 <ul className="absolute top-8 left-0 bg-white text-black shadow-lg rounded-md hidden group-hover:block min-w-[160px] z-50">
//                   {item.subcategories.map((sub, i) => (
//               <li key={i} className="px-4 py-2 hover:bg-gray-100 text-sm">
//                 <Link to={`/category/${item.slug}/${sub.slug}`}>{sub.name}</Link>
//                   </li>
//                   ))}
