...
const CheckoutPage = () => {
    ...
    if (val.length === 6) {
      try {
        ...
        if (data.Status === "Success") {
          ...
          setAreaSuggestions(data.PostOffice.map((p) => p.Name));
          setErrorMsg("");
        } else {
          setErrorMsg("❌ Invalid Pincode");
        }
  ...
  const handleAddressSubmit = async (e) => {
    ...
    try {
      if (isEditMode) {
        await axios.put(`${API_URL}/addresses/${editId}`, form, {
          headers: { Authorization: `Bearer ${token}` },
        });
      } else {
        await axios.post(`${API_URL}/addresses`, form, {
          headers: { Authorization: `Bearer ${token}` },
        });
      }
  ...
  return (
    <div className="max-w-3xl mx-auto px-6 py-10">
      ...
      {/* FORM */}
      <form onSubmit={handleAddressSubmit} className="grid grid-cols-2 gap-4 mb-8">
        <input value={form.fullName} onChange={(e)=>setForm({...form,fullName:e.target.value})} placeholder="Full Name" required className="border p-2"/>
        <input value={form.phone} onChange={(e)=>setForm({...form,phone:e.target.value})} placeholder="Phone" required className="border p-2"/>
        <input value={form.pincode} onChange={handlePincodeChange} placeholder="Pincode" required className="border p-2"/>
        <input value={form.state} readOnly className="border p-2 bg-gray-100"/>
        <input value={form.city} readOnly className="border p-2 bg-gray-100"/>
        <input value={form.addressLine} onChange={(e)=>setForm({...form,addressLine:e.target.value})} placeholder="Address" className="border p-2 col-span-2"/>
        <button className="col-span-2 bg-pink-600 text-white py-2 rounded">
          {isEditMode ? "Update Address" : "Save Address"}
        </button>
      </form>
      ...
      {/* LIST */}
      {addresses.map((addr) => (
        <div key={addr._id} className="border p-4 mb-3 rounded">
          <input type="radio" onChange={()=>setSelectedAddressId(addr._id)} />
          <span className="ml-2">{addr.addressLine}</span>
          <div className="mt-2 flex gap-3">
